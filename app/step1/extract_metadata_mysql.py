...
if not MYSQL_USER or MYSQL_PASS is None or not MYSQL_DB:
    raise ValueError("Defina MYSQL_USER, MYSQL_PASS e MYSQL_DB no arquivo .env")

MYSQL_PASS_ENC = urllib.parse.quote_plus(MYSQL_PASS)
conn_string = f"mysql+pymysql://{MYSQL_USER}:{MYSQL_PASS_ENC}@{MYSQL_HOST}:{MYSQL_PORT}/{MYSQL_DB}"

# =========================================================
# Flags de controle (mesmas)
# =========================================================
...
def save_progress(state, total_tables, current_run_started_at, current_run_wall_seconds):
    ...
    average_seconds_per_processed_table = (
        round(accumulated_table_seconds / processed_tables, 2)
    ...
    run_stats = {
        ...
        "average_seconds_per_processed_table": average_seconds_per_processed_table
    }

    atomic_write_json(RUN_STATS_PATH, run_stats)

# =========================================================
# Conexão e listagem de tabelas (adaptado para MySQL)
# =========================================================
...
def list_tables(inspector):
    ...
    for schema in inspector.get_schema_names():
        if schema in ("mysql", "information_schema", "performance_schema", "sys"):
            continue
...
# =========================================================
# Estatísticas avançadas de coluna (adaptado para MySQL)
# =========================================================
def get_column_advanced_stats(conn, schema, table, column_info, row_count):
    ...
    try:
        ...
        stats["null_count"] = int(result.null_count) if result.null_count is not None else 0
        stats["distinct_count"] = int(result.distinct_count) if result.distinct_count is not None else 0
...
# =========================================================
# Extração de metadados por tabela (adaptado para MySQL)
# =========================================================
def extract_table_metadata(inspector, conn, schema, table):
    ...
    row_count = 0
    try:
        q_count = text(f'SELECT COUNT(*) FROM `{table}`')
    ...
    if SAMPLE_ROWS > 0 and row_count > 0:
        try:
            qs = text(f'SELECT * FROM `{table}` LIMIT :lim')
