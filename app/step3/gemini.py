...
def call_gemini(client: genai.Client, prompt: str, system_prompt: str, temperature: float = TEMPERATURE) -> str:
    """
    Chama o Gemini usando google-genai, com system_instruction e temperatura configuráveis.
    Equivalente ao call_mistral() do script original.
    """
    text, _, _ = call_gemini_with_meta(client, prompt, system_prompt, temperature)
    return text


def call_gemini_with_meta(
    client: genai.Client,
    prompt: str,
    system_prompt: str,
    ...
    """
    Igual a call_gemini, mas retorna (texto, tokens_entrada, tokens_saida).
    tokens_entrada e tokens_saida são int (0 se a API não reportar).
    """
...
def llm_suggest_topics(client: genai.Client, table_summary: str) -> List[str]:
...
```"""
    ...
    content = call_gemini(client, prompt, system_prompt, temperature=TEMPERATURE)
...
def llm_suggest_topics_with_meta(
    client: genai.Client, table_summary: str
) -> tuple:
    """
    Igual a llm_suggest_topics, mas retorna (topics, tokens_entrada, tokens_saida).
    Usa call_gemini_with_meta internamente para capturar o uso da API.
    """
    system_prompt = "Você rotula tabelas de dados com temas de pesquisa."
    # reutiliza exatamente o mesmo prompt de llm_suggest_topics
    prompt = f"""
    Com base na descrição e metadados da tabela abaixo, liste até 3 temas principais de pesquisa ou análise que essa tabela pode ajudar a estudar.
...
```"""
    ...
    content, tokens_in, tokens_out = call_gemini_with_meta(
        client, prompt, system_prompt, temperature=TEMPERATURE
    )
