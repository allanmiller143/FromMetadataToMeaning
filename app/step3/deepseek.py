...
def call_deepseek(client: OpenAI, prompt: str, system_prompt: str, temperature: float = TEMPERATURE, max_tokens: int = 180) -> str:
    text, _, _ = call_deepseek_with_meta(client, prompt, system_prompt, temperature, max_tokens)
    return text


def call_deepseek_with_meta(client: OpenAI, prompt: str, system_prompt: str, temperature: float = TEMPERATURE, max_tokens: int = 180) -> tuple:
    """Igual a call_deepseek, mas retorna (texto, tokens_entrada, tokens_saida)."""
...
def llm_suggest_topics_with_meta(client: OpenAI, table_summary: str) -> tuple:
...
```"""
    ...
    content, tokens_in, tokens_out = call_deepseek_with_meta(
        client, prompt, system_prompt, temperature=TEMPERATURE, max_tokens=120
    )
